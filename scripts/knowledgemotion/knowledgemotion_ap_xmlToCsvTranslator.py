...
NO_LOWER_CASE_MATCHER = re.compile("^[^a-z]*$")
...
def main(arguments):
    parser = argparse.ArgumentParser(description='This is a script which takes a path to a top-level directory of XML files of AP metadata from KnowledgeMotion and outputs a .csv file which can be ingested by the Owl Atlas Processing ingest task by uploading it to the appropriate S3 bucket.')
        ...
        for xmlFile in files:
            ...
            if not bodyContentTag is None:
                for pTag in bodyContentTag.findAll('p'):
                    if pTag.string.startswith("Disclaimer:") or \
                    NO_LOWER_CASE_MATCHER.match(pTag.string) is None:
