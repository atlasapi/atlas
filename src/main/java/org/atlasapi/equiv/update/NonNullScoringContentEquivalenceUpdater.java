...
public class NonNullScoringContentEquivalenceUpdater<T extends Content> implements EquivalenceUpdater<T> {
    ...
    public static final class Builder<T extends Content> {
        ...
        public NonNullScoringContentEquivalenceUpdater<T> build() {
            return new NonNullScoringContentEquivalenceUpdater<T>(
                    generators.build(),
                    scorers.build(),
                    combiner,
                    filter,
    ...
    private NonNullScoringContentEquivalenceUpdater(
    ...
    public boolean updateEquivalences(T content) {
        ...
        removeGeneratorScoreForAllNullScoringContent(scoredScores, mergedScores);
    ...
    private void removeGeneratorScoreForAllNullScoringContent(
            List<ScoredCandidates<T>> scoredScores,
            List<ScoredCandidates<T>> mergedScores
    ) {
        for (int i = 0; i < scoredScores.size(); i++) {
            ScoredCandidates<T> scoredScore = scoredScores.get(i);
            ScoredCandidates<T> mergedScore = mergedScores.get(i);

            mergedScores.remove(i);
            if (allScoredNull(scoredScore)) {
                mergedScores.add(i, scoredScores.get(i));
            } else {
                mergedScores.add(i, mergedScore);
            }
        }
    }

    private boolean allScoredNull(ScoredCandidates<T> scoredScore) {
        for (T candidate : scoredScore.candidates().keySet()) {
            if (scoredScore.candidates().get(candidate).isRealScore()) {
                return true;
            }
        }
        return false;
